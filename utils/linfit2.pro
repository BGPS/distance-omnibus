...
;         Modified:  CT, RSI, July 2000: Added COVAR, MEASURE_ERRORS, YFIT
;                    keywords.
;         Modified:  TPEB, July 2011: Added NAN keyword to all TOTAL()
;                    functions to gaurd against bad data.
;-

FUNCTION LinFit2, x, y, $
...
; for explanation of constants see Numerical Recipes sec. 15-2
  if nsdev eq nX then begin ;Standard deviations are supplied.
	...
	ss = TOTAL(wt, DOUBLE=double, /NAN)
	sx = TOTAL(wt * x, DOUBLE=double, /NAN)
	sy = TOTAL(wt * y, DOUBLE=double, /NAN)
    t =  (x - sx/ss) / sdev
    b = TOTAL(t * y / sdev, DOUBLE=double, /NAN)
  endif else begin
	ss = nX
	sx = TOTAL(x, DOUBLE=double, /NAN)
	sy = TOTAL(y, DOUBLE=double, /NAN)
    t = x - sx/ss
    b = TOTAL(t * y, /NAN)
  ...
  if nsdev ne 0 then begin
    chisqr = TOTAL( ((y - yfit) / sdev)^2, Double = Double ,/NAN)
    if ~Double then $
        chisqr = FLOAT(chisqr)
    prob = 1 - IGAMMA(0.5*(nX-2), 0.5*chisqr)
  endif else begin
    chisqr = TOTAL( (y - yfit)^2, Double = Double , /NAN)
    if ~Double then $
        chisqr = FLOAT(chisqr)
    prob = double ? 1d : 1.0
