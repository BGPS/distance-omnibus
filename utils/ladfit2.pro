;$Id: //depot/Release/ENVI50_IDL82/idl/idldir/lib/ladfit.pro#2 $
;
...
;
; OPTIONAL OUTPUTS:
;  COVAR:     The (approximate) covariance matrix.
;
; EXAMPLE:
...
;   Modified: TPEB, 6/5/14: Add optional return of covariance matrix.
;-
...
;-------------------------------------------------------------------------
FUNCTION LadFit2, xIn, yIn, absdev = absdev, Double = DoubleIn, COVAR=covar
  ON_ERROR, 2
...
;  math in DP
  ...
  ;; Compute Covariance at this point for gits and shiggles; TPEB 6/5/14
  t = x - sx/nX
  st2 = TOTAL(t*t, DOUBLE=double, /NAN)
  
  sdeva = SQRT((1.0 + sx * sx / (nX * st2)) / nX)
  sdevb = SQRT(1.0 / st2)
  covar = -sx/(nX*st2)
  covar = [[sdeva*sdeva, covar], [covar, sdevb*sdevb]]
  ;; Resume your regularly-scheduled code...
