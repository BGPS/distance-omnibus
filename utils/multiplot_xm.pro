...
;   2011-06-20
;        Includes XMARGIN keyword to set equal-width margins on both
;        L & R sides of the plot set.  T. Ellsworth-Bowers
;   2012-10-24
;        Update to match latest multiplot.pro in the idlastro
;        distribution.  T. Ellsworth-Bowers
;
;-

PRO multiplot_xm, pmulti, help=help, $
        ...
        xmargin=xmargin, mpcharsize=mpcharsize, $
        hardleft=hardleft, hardright=hardright
    ...
    if init or keyword_set(initialize) then begin
        ...
        IF n_elements(xmargin) NE 0 THEN !x.window[1] = 1. - !x.window[0]
        IF KEYWORD_SET(hardleft) THEN !x.window[0] /= 2.
        IF KEYWORD_SET(hardright) THEN !x.window[1] = 1. - (1.-!x.window[1])/2.
